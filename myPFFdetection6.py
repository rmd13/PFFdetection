...
First Created on Fri Jun 11 13:11:15 2021
    ...
    #1.Installization
        ...
        install python and spyder3 (and pyimagej by way B2)
            ...
            --way B2:(suggested) download the python zip file Python36.rar containig both python, Spyder3 and pyimagej.
                ...
                then open 'C:\\Users\~\AppData\Local\Programs\Python\Python36\Scripts\spyder3.exe' in notepad++(download if not have)
                    find string 'rmd', you will locate to a path to pythonw.exe
...
idxCol_minorEllipsoid = 10;
    # = 0-based index of Column "Minor" in PFF_mask ROI measurement Results table. Must pre-set default measure option and get beforahnd.
...
idxCol_length = 4;
    # = 0-based index of Column "Perim." in PFF_mask ROI measurement Results table.
idxCol_area = 0;
    # = 0-based index of Column "Area" in Results table.
inTriExtruDistMinThresh = 4;
pixel_nm = 200/240; # scale bar real length/pixels
##################--dead parameters-#####################
aDiameterBuff = 0;
###################--imports--#######################
...
def process_one_image(image_url):
    if False:
        image_url = 'D:\\Lab\\LKC\\BarbaraInternshipProject\\test\\c.jpg'
    ...
    for iRoi in range(nRoiCount):
        ...
        if False:
            ...
            PFF_perimeter = (struct.unpack('<f',singleBytes))[0] * pixel_nm
            aAreaValue =  aResultImg.getPixel(int(idxCol_area),0)
            singleBytes = aAreaValue.to_bytes(4,'little')
            PFF_area = (struct.unpack('<f',singleBytes))[0] * pixel_nm * pixel_nm

        if PFF_area == 0 or PFF_perimeter == 0: # strange roi, skip
            continue
        PFF_perimeter_area_ratio = PFF_perimeter/PFF_area
        print("PFF_perimeter_ = " + str(PFF_perimeter) + ' nm')
        print("PFF_area = "+ str(PFF_area) + ' nm^2')
        ...
        print("PFF_isoLenSum = " + str(PFF_isoLenSum) + ' nm')
        print("PFF_isoThick = " + str(PFF_isoThick) + ' nm')
        ...
        singleBytes = aPixelValue.to_bytes(4,'little')
            #singleBytes = int(aResultImg.getPixel(10,0)).to_bytes(4,'little')
        PFF_EllipsoidMinorLen = (struct.unpack('<f',singleBytes))[0] * pixel_nm   #little endian. test Correct!
        ...
        print("PFF_EllipsoidMinorLen = " + str(PFF_EllipsoidMinorLen) + ' nm')
        print("PFF_nCollate = " + str(PFF_nCollate))
        print("PFF_FragIsoLen = " + str(PFF_FragIsoLen) + ' nm')
        
        ######--mask TEM by ips_PFFtrim then detect ridge
        # smooth again to reduce fragments
        aGaussianBlur = GaussianBlur();
        aGaussianBlur.setup("",ips_raw_defer_crop)
        ip = ips_raw_defer_crop.getProcessor();
        aGaussianBlur.blur(ip,GaussSigma) 
        ips_raw_defer_crop.updateAndDraw();
        if True:
            ...
            ips_raw_defer_crop_masked.deleteRoi() # remove constriction
            # then do full smooth. 
            if True:
                aGaussianBlur = GaussianBlur();
                aGaussianBlur.setup("",ips_raw_defer_crop_masked)
                ip = ips_raw_defer_crop_masked.getProcessor();
                aGaussianBlur.blur(ip,GaussSigma*2) #This smooth level has dramatic effect on ridge detection!!! from none, to clean, to overdetection.
                ips_raw_defer_crop_masked.updateAndDraw();
            else:
            ...
            # display results add_to_manager 
            # option for Ridge_detection. the line_width also has dramatic effect on ridge detection!!!
            #aOption = "line_width=3.5 high_contrast=230 low_contrast=87 method_for_overlap_resolution=SLOPE sigma=1.51 lower_threshold=3.06 upper_threshold=7.99 minimum_line_length=20 maximum=0"
            aOption = "line_width=3 high_contrast=230 low_contrast=20 method_for_overlap_resolution=NONE sigma=0.4 lower_threshold=3.06 upper_threshold=7.99 minimum_line_length="+ str(minimum_line_length)+  " maximum=0"
            if False:
               ...
               # aLinesObj.addToRoiManager = False #failed because fiels/method same name,field is private, but I switch to public thus confliet. Can access as method only.
                ...
                aLineLenAss,aLineIDAss,aLineTypeAss,aLineLenAss_noJunc,aLineIDAss_noJunc = [],[],[],[],[]
                for iLine in range(LineAss.size()):
                    aLine = LineAss.get(iLine)
                    aLineIDAss.append(aLine.getID())
                    aType = str(aLine.getContourClass())
                    print(aType)
                    aLineTypeAss.append(str(aLine.getContourClass))
                    aLineLenAss.append(aLine.estimateLength()*pixel_nm)
                    if True: # (aType == "cont_no_junc") or (aType == "cont_both_junc"):
                        aLen = aLine.estimateLength()*pixel_nm
                        if aLen > minimum_line_length:
                            aLineIDAss_noJunc.append(aLine.getID())
                            aLineLenAss_noJunc.append(aLen)
                # determine junc     
                JuncAss = aLinesObj.resultJunction[0]
                aJuncLvec,aJuncRvec,aJuncLRvec = [],[],[]
                for iJunc in range(JuncAss.size()):
                    aJunc = JuncAss.get(iJunc)
                    aJuncLvec = aJunc.getLine1().getID()
                    aJuncRvec = aJunc.getLine2().getID()
                    aJuncLRvec.append((aJuncLvec,aJuncRvec))
                # group junc into clusters  and sum length fragments  
                if False and len(aJuncLRvec)>0:
                    JuncGroups = get_cliques(aJuncLRvec)
                    aJuncGrpLenAss = []
                    for iGrp in range(len(JuncGroups)):
                        aJuncGrpSet = JuncGroups[iGrp] #set
                        aJuncGrpLen = 0
                        for aFragID in aJuncGrpSet:
                            aIDhit = np.where(aFragID==aLineIDAss)[0] # failed
                            if len(aIDhit)==0:
                               aJuncGrpLen = 0
                               break
                            else:
                               aFragLen = aLineLenAss[aIDhit]
                               aJuncGrpLen = aJuncGrpLen + aFragLen
                        if aJuncGrpLen > 0: 
                            aJuncGrpLenAss.append(aJuncGrpLen)
                    if len(aJuncGrpLenAss)>0:
                        aLineLenAss_noJunc = aLineLenAss_noJunc.append(aJuncGrpLenAss)
                ...
                with open(aCsvPath, 'w', newline='') as csvfile:
                    ...
                    for iNum in range(len(aLineLenAss_noJunc)):
                        spamwriter.writerow([str(aLineLenAss_noJunc[iNum])])
