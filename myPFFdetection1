...
##################--parameters-#####################
aFmt = '.tif' # extension of input image
GaussSigma = 1.5
aThresholdMtd = "Triangle dark" #"MaxEntropy dark"
nFatigueMax = 30; #10 by default
...
def process_one_image(image_url):
    ...
    for iRoi in range(nRoiCount):
        ...
        aRoiPath = os.path.splitext(image_url)[0]+"_Roi_"+ str(iRoi) +".png"
        ...
        PFF_perimeter = (aMatrix_PFF.sum() - aMatrix_PFF2.sum())/255
        PFF_area = aMatrix_PFF.sum()/255
        ...
        aTilePngPath = os.path.splitext(image_url)[0]+"_RoiOverlay_"+ str(iRoi) +".png"
        ...
        aCsvPath = os.path.splitext(image_url)[0]+"_RoiCrop_"+ str(iRoi) + '.csv'
        with open(aCsvPath, 'w', newline='') as csvfile:
            spamwriter = csv.writer(csvfile, delimiter=',',quotechar='"', quoting=csv.QUOTE_MINIMAL)
            spamwriter.writerow(['PFF_perimeter_',PFF_perimeter])
